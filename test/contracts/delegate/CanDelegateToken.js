...
const GOLDTokenMock = artifacts.require("./GOLDTokenMock.sol");
...
function check(accounts, deployTokenCb) {
  ...
  var tokenName = "GOLDTokenMock";
  var tokenSymbol = "GOLDMock";
...
//    await otherToken.mint(purchaser, balanceOtherToken).should.be.fulfilled;
  ...
  describe('delegateToNewContract()', function() {
    beforeEach(async function () {
      ...
      newToken = await GOLDTokenMock.new(tokenName, tokenSymbol, publicDoc, systemWallet, _newTotalSupply);
    });
